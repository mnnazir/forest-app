...
function AnalyzePage() {
  ...
  const getCarbonStorage = (potential) => {
    switch (potential.toLowerCase()) {
      case 'high': return "150-200 tons/ha";
      case 'medium': return "100-150 tons/ha";
      case 'low': return "50-100 tons/ha";
      default: return "100-150 tons/ha";
    }
  };
  ...
  const handleAnalyze = async (e) => {
    ...
    try {
      ...
      // 3. Prepare data for database (MATCHING DASHBOARD STRUCTURE)
      const dbData = {
        user_id: user.id,
        // ✅ Dashboard ke hisaab se columns:
        image_url: preview || "no-image.jpg",
        biodiversity: aiAnalysis.biodiversity_score * 10, // Convert to 0-100 scale
        health: getHealthCategory(aiAnalysis.health_score),
        species: aiAnalysis.species || [],
        carbon_storage: getCarbonStorage(aiAnalysis.carbon_potential),
        analysis_data: {
      ...
      // 5. Set results and show success
      setAiResults({
        ...finalResult,
        id: data[0]?.id,
        date: new Date().toLocaleDateString('en-US', { 
      ...
      setActiveTab('results');
      setSaveMessage(`✅ Forest analysis saved to dashboard! (ID: ${data[0]?.id?.slice(0, 8) || 'N/A'})`);
      ...
      // Fallback: Still show analysis results (without saving)
      const aiAnalysis = getSimulatedAI();
      const technicalScore = (
        aiAnalysis.health_score * 2 + 
        aiAnalysis.biodiversity_score * 1.5 +
    ...
    <div className="analyze-page-container">
      ...
      {saveMessage && (
        <div className="save-success-message">
          ...
          {saveMessage}
          <button onClick={() => setSaveMessage('')} className="close-message">✕</button>
        </div>
      )}
      ...
      {saveError && (
        <div className="save-error-message">
          ...
          {saveError}
          <button onClick={() => setSaveError('')} className="close-message">✕</button>
        </div>
      )}
          ...
          {activeTab === 'values' && (
            <div className="values-tab">
              <div className="values-container">
                ...
                <div className="database-note">
                  <p>💾 Your analysis will be saved to your personal dashboard</p>
                  <p className="note-details">Data structure matched with dashboard requirements</p>
                </div>
              </div>
            </div>
          )}
              ...
              <div className="results-container">
                <div className="results-header">
                  ...
                  <div className="save-status">
                    {saveMessage ? (
                      ...
                      <div className="status-success">
                        <span className="status-icon">✅</span>
                        Saved to Dashboard (ID: {aiResults.db_id.slice(0, 8)}...)
                      </div>
                    ) : (
                      <div className="status-warning">
                        <span className="status-icon">⚠️</span>
                        Not saved to dashboard (login required)
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="ai-details">
                    ...
                    <div className="ai-grid">
                      ...
                      <div className="ai-metric">
                        ...
                        <div className="metric-value">{aiResults.ai.biodiversity_score}/10</div>
                      </div>
                ...
                <div className="data-structure-card">
                  ...
                  <div className="structure-content">
                    <p><strong>Saved to Dashboard with these fields:</strong></p>
                    <ul className="structure-list">
                      <li>🌿 <strong>Biodiversity:</strong> {aiResults.ai.biodiversity_score * 10}/100</li>
                      <li>💚 <strong>Health:</strong> {getHealthCategory(aiResults.ai.health_score)}</li>
                      <li>🌳 <strong>Species:</strong> {aiResults.ai.species.slice(0, 3).join(', ')}...</li>
                      <li>🌍 <strong>Carbon Storage:</strong> {getCarbonStorage(aiResults.ai.carbon_potential)}</li>
                      <li>📁 <strong>Full Data:</strong> Saved in <code>analysis_data</code> column</li>
                    </ul>
                  </div>
                </div>
